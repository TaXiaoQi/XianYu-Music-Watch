...
mod tests {
    ...
    fn build_cenc_m4a(samples: &[Vec<u8>], ivs: &[Vec<u8>]) -> Vec<u8> {
        ...
        enca_payload.extend_from_slice(&[0u8; 8]);
        enca_payload.extend_from_slice(&[0u8; 20]);
        let mut tenc_payload = Vec::new();
        tenc_payload.extend_from_slice(&[0u8; 4]);
        tenc_payload.push(0);
        tenc_payload.push(0);
        tenc_payload.push(1);
        tenc_payload.push(ivs[0].len() as u8);
        tenc_payload.extend_from_slice(&[0u8; 16]);
